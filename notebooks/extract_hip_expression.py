"""Extract HPF expression, generate dotplot, commit+push."""
...
outdir = Path('scube1_hip')
outdir.mkdir(exist_ok=True)
roi_cells.to_csv(outdir / 'hip_metadata.csv')
...
git_cp(['notebooks/scube1_hip/hip_metadata.csv'], f'Add {ROI} metadata CSV')

# HPF is in WMB-10Xv2-HPF and WMB-10Xv3-HPF
...
combined.to_csv(outdir / 'hip_expression.csv')
print(f"Saved expression CSV ({combined.shape})")
git_cp(['notebooks/scube1_hip/hip_expression.csv'],
       f'Add HPF expression CSV ({combined.shape[0]:,} cells)')

# === Dotplot ===
print("Generating dotplot...")
expr = pd.read_csv(outdir / 'hip_expression.csv', index_col=0)
meta = pd.read_csv(outdir / 'hip_metadata.csv', index_col=0)
...
noise_p = [
    ...
    'OB ','OB-','Astro-OLF','OEC',
    # Thalamic
    'TH Prkcd','RT-ZI','LGv','PVT',
    # Hypothalamic
    'LHA ','LHA-','AHN ','DMH ','VMH ','PVH','HY ','AVPV','MPO-ADP',
    'SBPV','PH-',
    # Amygdala
    'CEA-','MEA-','MEA ','LA-BLA','COAa-PAA',
    # Midbrain
    'PAG','SC ','SNc-VTA','MRN','IC ','SCiw','SCig',
    # Septal
    'LSX ','NDB-SI','MS-SF','BST-MPN','BST-SI',
    # Cortical (noise from adjacent cortex - but keep entorhinal which borders HIP)
    'L6 CT CTX','IT EP-CLA','CLA-EPd','IT AON',
    'Pvalb chandelier',
    # Other noise
    'DC NN','Lymphoid','Monocytes','ABC NN',
    'OB-STR-CTX','PVR ','SI-MA',
]
...
fig.savefig(outdir / 'dotplot_HPF_Scube1_by_subclass.png', dpi=150, bbox_inches='tight')
plt.close()
print('Saved dotplot')
git_cp(['notebooks/scube1_hip/dotplot_HPF_Scube1_by_subclass.png', 'notebooks/extract_hip_expression.py'],
       'Add Scube1 dotplot for cortical subplate (HPF / BLA)')
print("Done!")
