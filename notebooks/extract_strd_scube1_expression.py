#!/usr/bin/env python3
"""
Extract Scube1 + cell marker expression for dorsal striatum from ABC Atlas 10x.

Target genes:
  - Scube1: gene of interest
  - Drd1, Drd2: D1/D2 MSN markers
  - Ppp1r1b (DARPP-32): pan-MSN marker
  - Chat: cholinergic interneuron marker
  - Sst, Pvalb: interneuron markers
  - Gad1, Gad2: GABAergic markers
  - Slc17a7 (Vglut1): glutamatergic marker
  - Th: dopaminergic/catecholaminergic marker
  - Adora2a: D2-MSN / indirect pathway marker

Region: STRd (dorsal striatum)
"""
import os
import gc
import time
...
output_csv = Path('strd_scube1_expression.csv')
output_meta = Path('strd_scube1_metadata.csv')
...
# --- Identify dorsal striatum cells ---
strd_cells = cell_extended[cell_extended['region_of_interest_acronym'] == 'STRd'].copy()
print(f"\nDorsal striatum (STRd) cells: {len(strd_cells):,}")

# Save metadata
strd_cells[['subclass', 'supertype', 'class', 'neurotransmitter',
...
strd_matrices = strd_cells.groupby('feature_matrix_label').size()
print(f"\nExpression matrices to process: {len(strd_matrices)}")
for mat, count in strd_matrices.items():
    print(f"  {mat}: {count:,} cells")

expression_frames = []

for i, matrix_label in enumerate(strd_matrices.index):
    dataset_label = strd_cells[
        strd_cells['feature_matrix_label'] == matrix_label
    ]['dataset_label'].iloc[0]
    file_name = f"{matrix_label}/log2"

    print(f"\n[{i+1}/{len(strd_matrices)}] Processing {file_name}...")
    t0 = time.time()
    ...
    with h5py.File(file_path_str, 'r') as f:
        ...
        strd_cell_label_set = set(
            strd_cells[strd_cells['feature_matrix_label'] == matrix_label].index
        )
        cell_idx = np.array([j for j, name in enumerate(obs_cell_labels)
                             if name in strd_cell_label_set])
        ...
        print(f"  STRd cells found: {len(cell_idx):,}")
        ...
        if len(cell_idx) == 0:
            ...
            # Pre-allocate output array: n_target_cells x n_target_genes
            ...
            # Process in chunks of cells to limit memory
            chunk_size = 5000
