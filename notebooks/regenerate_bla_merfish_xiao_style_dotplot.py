#!/usr/bin/env python3
"""
BLA MERFISH companion to the BLA 10x Xiao-style dot plot.

MERFISH gene panel only includes 3 of the 7 5-HT receptors used in the 10x
plot: Htr1b, Htr2a, Htr7. Htr1a, Htr2b, Htr2c, and Htr4 are NOT measured.
Same subclasses and styling as the 10x version.

Run: python notebooks/regenerate_bla_merfish_xiao_style_dotplot.py
"""
import re
...
EXPR_CSV = NOTEBOOK_DIR / 'bla_merfish_expression.csv'
META_CSV = NOTEBOOK_DIR / 'bla_merfish_metadata.csv'
...
# Of the 7 receptors used in 10x, only these 3 are in the MERFISH panel.
GENES = ['Htr1b', 'Htr2a', 'Htr7']
...
def main():
    ...
    print(f'BLA cells used: {len(meta_sel):,}')
    ...
    print('\nMean (MERFISH normalized counts):')
    print(mean_expr.round(2).to_string())
    print('\nFraction expressing:')
    print(frac_expr.round(2).to_string())

    # MERFISH values are not log2(CPM+1); use the data range for scaling.
    vmax = float(np.percentile(mean_expr.values, 99))
    vmax = max(vmax, 1.0)
    print(f'\nColor vmax set to {vmax:.2f} (99th percentile of subclass means)')

    n_rows = len(TARGET_SUBCLASSES)
    n_cols = len(GENES)

    fig = plt.figure(figsize=(0.95 * n_cols + 5.5, 0.55 * n_rows + 2.5))
    ax = fig.add_axes([0.20, 0.15, 0.45, 0.72])
    ...
    ax.set_title('BLA — Zhuang MERFISH', fontsize=11)
    ...
    cbar.set_label('Mean MERFISH\ncounts (norm.)', fontsize=9)
    cbar.ax.tick_params(labelsize=8)

    out = OUTPUT_DIR / 'dotplot_BLA_MERFISH_xiao_style.png'
