#!/usr/bin/env python3
"""
Extract receptor gene expression for mPFC glia (microglia + astrocytes) from ABC Atlas.
Same approach as extract_mpfc_expression.py but for non-neuronal cell types.
"""
import os
import gc
import time
...
output_csv = Path('mpfc_glia_receptor_expression.csv')
...
glia_selected = mpfc_cells[mpfc_cells['subclass'].isin(glia_subclasses)].copy()
print(f"mPFC glia cells: {len(glia_selected):,}")
for sc, count in glia_selected.groupby('subclass', observed=True).size().items():
    print(f"  {sc}: {count:,}")
...
# Extract expression
glia_matrices = glia_selected.groupby('feature_matrix_label').size()
print(f"\nExpression matrices to process: {len(glia_matrices)}")
for mat, count in glia_matrices.items():
    print(f"  {mat}: {count:,} cells")

expression_frames = []

for i, matrix_label in enumerate(glia_matrices.index):
    dataset_label = glia_selected[
        glia_selected['feature_matrix_label'] == matrix_label
    ]['dataset_label'].iloc[0]
    file_name = f"{matrix_label}/log2"

    print(f"\n[{i+1}/{len(glia_matrices)}] Processing {file_name}...")
    t0 = time.time()
    ...
    with h5py.File(file_path_str, 'r') as f:
        ...
        glia_cell_label_set = set(
            glia_selected[glia_selected['feature_matrix_label'] == matrix_label].index
        )
        cell_idx = np.array([j for j, name in enumerate(obs_cell_labels)
                             if name in glia_cell_label_set])
        ...
        print(f"  Glia cells found: {len(cell_idx):,}")
...
print(f"\nSaved: {expression_data.shape[0]:,} cells x {expression_data.shape[1]} genes -> {output_csv}")
